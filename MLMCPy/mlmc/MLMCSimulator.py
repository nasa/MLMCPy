...
class MLMCSimulator(object):
    ...
    def get_model_inputs_to_run_for_each_level(self, sample_sizes):
        """
        Takes optimal sample sizes generated by compute_optimal_sample sizes and
        returns a dictionary 2D np.ndarray with random inputs.
        ...
        inputs_dict = {}
        sample_sum = np.sum(sample_sizes)
        inputs = self._data.draw_samples(np.sum(sample_sum))
        ...
        for level in range(len(sample_sizes)):
            if level == 0:
                ...
                inputs_dict.update({'level'+str(level): \
                    inputs[sample_sum-sample_sizes[level]:]})

            else:
                inputs_dict.update({'level'+str(level): \
                    inputs[sample_sizes[level-1]:]})
    ...
    def store_model_inputs_to_run_for_each_level(self, sample_sizes,
        ...
        if filenames is not None:
            for counter, key in enumerate(inputs):
                np.savetxt('%s' % filenames[counter], inputs[key])
                counter += 1
        else:
            for key in inputs:
    ...
    def __check_get_model_inputs_to_run_for_each_level_params(sample_sizes):
        if not isinstance(sample_sizes, list):
            raise TypeError('sample_sizes must be a list generated by' +
                            'compute_optimal_sample_sizes().')
    ...
    def __check_store_model_params(sample_sizes, filenames=None):
        ...
        if filenames is not None:
            if not isinstance(filenames[0], str):
                raise TypeError('filenames must be a string.')
