...
def test_generate_wrapper_models_output_types(spring_models):
    """
    Ensures that the models that are generated by generate_wrapper_models_list are
    properly allocated, and maintain the correct class.
    """

    grid = np.arange(0., 1., .1)
    cdf_wrapper = CDFWrapperModel(grid)
    test_mlmc = MLMCSimulator.generate_wrapper_models_list(spring_models, cdf_wrapper)
