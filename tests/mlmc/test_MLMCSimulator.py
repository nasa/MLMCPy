...
from MLMCPy.model import ModelFromData, CDFWrapperModel, Model
...
def test_generate_models_exceptions(spring_models):
    grid = np.arange(0., 1., .1)
    cdf_wrapper = CDFWrapperModel(grid)

    with pytest.raises(TypeError):
        MLMCSimulator.generate_models_list('Not a Model', cdf_wrapper)
    
    with pytest.raises(TypeError):
        MLMCSimulator.generate_models_list(spring_models, 'Not a WrapperModel')

def test_generate_models_output_types(spring_models):
    """
    Ensures that the models that are generated by generate_models_list are
    properly allocated, and maintain the correct class.
    """
